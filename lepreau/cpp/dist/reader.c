...
int	nrecur;
int	tnumflag;		/* template number flag, ref'ed in match.c */
int	lflag;
int	edebug = 0;
int	xdebug = 0;
int	udebug = 0;
...
OFFSZ	maxoff;  /* max temp offset over all blocks in current ftn, in bits */
int	maxtreg;
...
codgen( p, cookie ) NODE *p; COOKSZ cookie; {
...
order(p,cook) NODE *p; COOKSZ cook; {

	register int	o, ty, m;
	int		m1;
	COOKSZ		cookie;
	NODE		*p1, *p2;
	extern COOKSZ	nextcook();
 ...
 nomat:
	...
	case ASG PLUS:  /* and other assignment ops */
		...
		p2 = tcopy(p);
		p2->op = NOASG p2->op;	   /* this was omitted in 11 & /6 !! */
		p->op = ASSIGN;
...
int negrel
[] = { NE, EQ, GT, GE, LT, LE, UGT, UGE, ULT, ULE } ;  /* negatives of relationals */
