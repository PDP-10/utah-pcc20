/* <COS.LEPREAU.CPP.DIST>CODE2.C.1, 22-Jan-86 17:35:38, Edit by BEEBE */
/* NHFB02: Add GENLABELS defined code to create labels for all variables */
/* to make debugging with DDT easier. */
...
#define GENLABELS 1	/* define to get vars into MACRO labels for DDT */

#define GENMODULES 1	/* define for separate module symbol tables */
#undef GENMODULES	/* DOES NOT WORK -- see --README.TXT */

#define REPEAT 1	/* define to get REPEAT macro instead of N */
			/* zeroes which fill up the output file */
...
#ifdef GENMODULES
char svfilnam[FNMAX+1];			/* use FNMAX from stdio.h */
#endif GENMODULES
...
#ifdef GENMODULES
	printf(")\f\ttitle\t%s\t\t;%s\n", exname(p->sname),
	    (*svfilnam == EOS) ? "stdin" : svfilnam);
	printf(")\tradix\t10\n");
	printf(")\tsearch\t$cdefs(pcc:$cdefs.unv)\n");
	printf(")\t.directive flblst,sfcond,kl10\n");
	printf(")\ttwoseg\t$text\n");
#endif GENMODULES
...
#ifdef GENLABELS
	printf( ")\n");
	printf( "); Begin Argument Variables\n");
#endif GENLABELS
...
#ifdef GENLABELS
			/* Names are prefixed with % to distinguish them as */
			/* offsets into the stack frame, as opposed to normal */
			/* names for register and static variables. */
			/* Since they are negative, DDT will not use them */
			/* in symbolic output */

			if (p->sclass == AUTO)
			    printf( ")%%%s=^D%d\t\t\t;AUTO variable\n",
			        exname(p->sname), (int)p->offset/SZINT);
			else if (p->sclass == SNULL)
			    printf( ")%%%s=^D%d\t\t\t;SNULL variable\n",
			        exname(p->sname), (int)p->offset/SZINT);
			else if (p->sclass == STNAME)
			    printf( ")%%%s=^D%d\t\t\t;STNAME variable\n",
			        exname(p->sname), (int)p->offset/SZINT);
			else if (p->sclass == PARAM)
			    printf( ")%%%s=^D%d\t\t\t;PARAM variable\n",
			        exname(p->sname), (int)p->offset/SZINT);
#endif GENLABELS
			}
...
char *filnam;
{
...
#ifdef GENMODULES
	strncpy(svfilnam,filnam,FNMAX);	/* save filename for bfcode() */
	svfilnam[FNMAX] = '\0';
#else
	if (*filnam == EOS)
	    p = "stdin";
	...
	{
	    if ((p = rindex(filnam, '/')) == NULL)
	        p = filnam;
	    else
	        p++;				/* strip off pathname */
	}
	printf(")\ttitle\t%s\t\t;%s\n", p, (*filnam == EOS) ? "stdin" : filnam);
#endif GENMODULES
