...
{
			umul:
				yyval.nodep = bdty( UNARY MUL, yypvt[-0].nodep, 0 );
			} break;
case 57:
# line 317 "p1/cgram.y"
{
			uftn:
				yyval.nodep = bdty( UNARY CALL, yypvt[-2].nodep, 0 );
			 } break;
case 58:
# line 322 "p1/cgram.y"
{
			uary:
				yyval.nodep = bdty( LB, yypvt[-2].nodep, 0 );
			 } break;
case 59:
# line 327 "p1/cgram.y"
{
...
# line 633 "p1/cgram.y"
{
				...
				if(!reached)
					werror( "loop not entered at top");
				if(yypvt[-1].nodep->op == ICON && yypvt[-1].nodep->lval != 0)
					flostat = FLOOP;
...
# line 691 "p1/cgram.y"
{
			preconf:
				if( yychar == RELOP || yychar == EQUOP ||
				   yychar == AND || yychar == OR || yychar == ER ){
			    precplaint:
					if(hflag)
...
case 147:
# line 779 "p1/cgram.y"
{  yyval.nodep=buildtree(QUEST, yypvt[-4].nodep, buildtree( COLON, yypvt[-2].nodep, yypvt[-0].nodep ) ); } break;
case 148:
# line 781 "p1/cgram.y"
...
# line 856 "p1/cgram.y"
{
				...
				if(yypvt[-0].intval)
					yyval.nodep->csiz = yyval.nodep->type = ctype(LONG);
			} break;
case 166:
# line 864 "p1/cgram.y"
{
