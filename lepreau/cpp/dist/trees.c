 	      ...
 	      *r;
{
  ...
  */
    ...
    else if ((o == ANDAND || o == OROR) && (l->op == ICON || r->op == ICON))
		    goto ccwarn;
	...
	ccwarn: 
		...
		/* 
		 * Need type conversions here, or conval will
		 * make expressions like (1L+1) into (integer)2.
		 * So, simulate part of tymatch. Leffler: still bug???
		 */
		    t1 = l->type, t2 = r->type, u = 0;
		    if (ISUNSIGNED (t1))
			u = 1, t1 = DEUNSIGN (t1);
		    if (ISUNSIGNED (t2))
			u = 1, t2 = DEUNSIGN (t2);
		    ...
		    if (t != t1)
			l = clocal (makety (l, tu, 0, (int) tu));
		    if (t != t2)
			r = clocal (makety (r, tu, 0, (int) tu));
		/* 
		 * Finally make sure that we haven't messed
		 * things up by inserting an extra node....
		 */
		    if ( (l->op != ICON || r->op != ICON) &&
		    	    (o != ANDAND && o != OROR) )
			cerror ("bad constant collapse");
		    if (conval (l, o, r)) {
			r->op = FREE;
			return (l);
		    }
...
TWORD type;
{
	...
	for (; (kk = dimtab[k]) >= 0; ++k) {
		...
		/* no recursive looking for strs */
		    if (chkstr (i, dimtab[stab[kk].sizoff + 1], stab[kk].stype))
			return (1);
	    }
	}
    }
         	 ...
         	 *q;
{
    ...
    if (p->rval != NONAME && q->rval != NONAME)
	return (0);
    if (q->rval != NONAME && o != PLUS)
	return (0);
    if (p->rval != NONAME && o != PLUS && o != MINUS)
	return (0);
 ...
 /* 
  * Be careful to coerce collection of byte and word
  * offsets, sym_x contains a tag as to BYTES, WORDS, or
  * NOTYPE...make sure attribute gets passed along.
  */
    if (p->sym_x == NOTYPE)
	p->sym_x = q->sym_x;
    return (1);
...
register    NODE *p;
{
	...
	if (t1 == ENUMTY && t2 == ENUMTY && p->left->csiz == p->right->csiz)
	    return;
...
register    NODE *p;
{
    ...
    r = block (PMCONV, q, bpsize (f == CVTL ? p->right : p->left), INT, 0, INT);
    r = clocal (r);
    if (f == CVTL)
	p->left = r;
    else
...
register    NODE *p;
{
	...
	case GE: 
	case GT: 
	    if (ISUNSIGNED (p->left->type) || ISUNSIGNED (p->right->type))
		p->op += (ULE - LE);
	case EQ: 
	case NE: 
	    return (p);
	...
	case MINUS: 
	    return (clocal (block (PVCONV, p, bpsize (p->left), INT, 0, INT)));
    }
...
register    NODE *p;
{
	...
	default: 			/* must work harder: relationals or
					   comparisons */
	    if (!ISPTR (t1)) {
		t = t2;
		d = d2;
...
register    NODE *p;
{
  ...
  */
    if (t != t1)
	p->left = makety (p->left, tu, 0, (int) tu);

    if (t != t2 || o == CAST)
	p->right = makety (p->right, tu, 0, (int) tu);
...
opact (p)		/* 'opact' lifted from 11 for format reasons */
NODE *p;
{
    register    mt12,
                mt1,
       ...
       *b;
{
    ...
    *a = *b = down + 1;
	...
	if (p->op == ICON)
	    printf (" (%s) ", p->sym_x == NOTYPE ? "NOTYPE" :
		    p->sym_x == BYTES ? "BYTES" : "WORDS");
	printf (", %d, ", p->rval);
    }
    tprint (p->type);
    printf (", %d, %d\n", p->cdim, p->csiz);
...
# ifdef MYPRTREE
    MYPRTREE (p);			/* local action can be taken here;
					   then return... */
#endif
  ...
  */
	...
	/* 
	 * print out size
	 * use lhs size, in order to avoid hassles with the structure
	 *  `.' operator
	 *
	 * note: p->left not a field...
	 */
	    printf (CONFMT, (CONSZ) tsize ((TWORD) STRTY, p->left->cdim,
		    p->left->csiz));
	    printf ("\t%d\t\n", talign ((TWORD) STRTY, p->left->csiz));
	    break;
...
# endif
	 ...
	 */
	    p->stsize = (tsize ((TWORD) STRTY, p->left->cdim, p->left->csiz) +
	    	    SZCHAR - 1) / SZCHAR;
	    p->stalign = talign ((TWORD) STRTY, p->left->csiz) / SZCHAR;
	    break;
