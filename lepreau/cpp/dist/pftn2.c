/* <COS.LEPREAU.CPP.DIST>PFTN2.C.1, 23-Jan-86 11:39:28, Edit by BEEBE */
/* Added GENLABELS code to get local variables into symbol table */
/*	pftn.c	 Slightly modified for Dec-20.	J.Lepreau  2/81	*/

#define GENLABELS 1	/* define for symbol table stuff */
#define GENMODULES 1	/* define for separate module symbol tables */
#undef GENMODULES	/* DOES NOT WORK -- see --README.TXT */

# include "mfile1"

struct instk {
	int	in_sz;		/* size of array element */
	int	in_x;		/* current idx for struct memb in struct inits */
...
int	ddebug = 0,
	idebug = 0,
	ibseen = 0,	/* the no. of } constructions which have been filled */
	iclass,		/* storage class of thing being initialized */
	ilocctr = 0;	/* location counter for current initialization */
...
#if DEBUG & PFTN
	if (ddebug > 2) {
	printf("pftn:defid(), node:\n");
	fwalk(q, eprint, 0);
	printf("stab entry: sname (%o) = %s, stype = %d, sclass = %d\n",
		p->sname, p->sname, p->stype, p->sclass);
	}
#endif
	...
	if (stp == UNDEF || stp == FARG) {
		if (blevel == 1 && stp != FARG)
			switch (class) {
				...
				default:
					if (!(class & FIELD))
						uerror ("declared argument %.8s is missing", p->sname);
				case MOS:
				case STNAME:
				case MOU:
				case UNAME:
...
enter: 				       /* make a new entry */
	...
	if (ddebug)
		printf ("	dimoff, sizoff, offset: %d, %d, %ld\n",
			p->dimoff, p->sizoff, p->offset); /* OFFSZ dependent */
...
#ifdef GENLABELS
	printf( ")\n");
	printf( "); Begin Local Variables\n");
	for (p = stab; p < &stab[SYMTSZ]; ++p)
	{
		if ((p->stype != TNULL) && (p->sclass != SNULL))
		{
		    if (p->sclass == AUTO)
			    printf( ")%%%s=^D%d\t\t\t;AUTO variable\n",
			        exname(p->sname), (int)p->offset/SZINT);
		    else if (p->sclass == STNAME)
			    printf( ")%%%s=^D%d\t\t\t;STNAME variable\n",
			        exname(p->sname), (int)p->offset/SZINT);
		    else if (p->sclass == MOS)
			    printf( ")%%%s=^D%d\t\t\t;MOS (Member of Structure) variable\n",
			        exname(p->sname), (int)p->offset/SZINT);
		    else if (p->sclass == MOE)
			    printf( ")%%%s=^D%d\t\t\t;MOE (Member of Enumeration) variable\n",
			        exname(p->sname), (int)p->offset/SZINT);
		    else
		        printf( ")\t\t\t\t;%s\tclass=%d\toffset=%d\n",
			    p->sname, p->sclass, p->offset);
		}
	}
	printf( "); End Local Variables\n");
	printf( ")\n");
#endif
...
register		s;
{
	...
	if (s < 0 && ty != INT && ty != CHAR && ty != SHORT &&
...
getstr()
{
	...
	if ((iclass == EXTDEF || iclass == STATIC) &&
		 ...
		 * and be prepared to throw away unwanted initializers
		 */
...
endinit()
{
	 ...
	 */
		...
		/*
		 * We kluge here, so that global chars will be
		 * placed in the rightmost byte, rather than in
		 * the leftmost (where pcc wants them).  The
		 * initkluge indicates to vfdzero (called through
		 * inforce) that it should update inoff, but not
		 * zero those bits ... this is done ONLY for those
		 * chars that are EXTDEF or STATIC.
		 */
		if (t == CHAR || t == UCHAR) {
			register struct symtab	*q;

			q = &stab[pstk->in_id];
			if (q->sclass == EXTDEF || q->sclass == STATIC)
				initkluge = 1;
		}
		inforce (tsize (t, d, s));
	}
...
# endif
#ifdef BACKPARM			/* TEMPORARY KLUDGE ONLY--(???)  fjl */
	if (p->sclass == PARAM) {
		noff = off + tsz;
		SETOFF (noff, al);
		off = -noff;
...
# ifndef RTOLBYTES
	...
	if (p->sclass != REGISTER) {
		/*
		 * in case we are allocating stack space for register
		 * arguments
		 */
...
{
	register	class;
	register	commflag = 0;  /* flag for labelled common declarations */
...
register NODE	*p;
{
	...
	t = INCREF (p->type);
	if (ddebug > 2)
		printf("tyreduce: after INCREF %o\n", t);
	if (o == UNARY CALL)
		t + = (FTN - PTR);
	else if (o == LB) {
		t + = (ARY - PTR);
		temp = p->right->lval;		/* array dimension in 'units'*/
		p->right->op = FREE;
	}
	p->left->type = t;
	tyreduce (p->left);
...
char   *name;
{
	...
	i = i % SYMTSZ;
#if DEBUG & PFTN
	if (ddebug > 2)	printf("pftn:lookup(), hash value = %d\n", i);
#endif
	sp = &stab[ii = i];
#if DEBUG & PFTN
	if (ddebug > 2) printf("symbol table entry = %o\n", sp);
#endif
	for (;;) {		       /* look for name */
		...
		if (sp->stype == TNULL) {	/* empty slot */
			p = sp->sname;
			/*
			 * set SMOS if needed, turn off all others
			 */
			sp->sflags = s;
			for (j = 0; j < NCHNAM; ++j)
				if (*p++ = *name)
...
#if DEBUG & PFTN
			if (ddebug > 2)
				printf("entered new id: j = %d, name: %s\n", j, sp->sname);
#endif
			return (i);
		}
		if ((sp->sflags & (SMOS | SHIDDEN)) != s)
			goto next;
		p = sp->sname;
		q = name;
		for (j = 0; j < NCHNAM; ++j) {
...
register struct symtab *p;
{
	...
	if (hflag)
		werror ("%.8s redefinition hides earlier one", p->sname);
	if (ddebug)
