/* Dec-20 version, slightly modified from pdp-11 one. */

# include "mfile2"

NODE resc[3];		/* Resource nodes, used in allocation strategy */

int busy[REGSZ];	/* Busy status of registers */
...
freetemp( k ){	/* Allocate k integers worth of temp space.		    */
		...
		/* is more than 1, it must be aligned for storing doubles...*/

#if DEBUG & ALLO
	if (rdebug > 2)	printf("freetemp(%d)\n", k);
#endif
...
reclaim( p, rw, cookie ) NODE *p; COOKSZ cookie; {
	register NODE **qq;
	register NODE *q;
	register i;
	NODE *recres[5];
 ...
 gotit:
	...
	case REG:
		if( !rtyflg ){
		/* The C lang. requires intermediate results to change type.*/
...
ncopy( q, p ) NODE *p, *q; {
	...
	q->lval = p->lval;
	q->rval = p->rval;
	for( i=0; i<NCHNAM+1; ++i ) q->name[i]  = p->name[i];
