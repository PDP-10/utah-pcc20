/* <COS.LEPREAU.CPP.DIST>CPP.C.5, 22-Apr-86 13:19:42, Edit by BEEBE */
/* Increase symbol size from 8 to NCHNAM (= 63) for compatibility with */
/* other Unixes */
/* <COS.LEPREAU.CPP.DIST>CPP.C.2, 13-Nov-85 14:30:56, Edit by BEEBE */
/* Increase symsiz from 1000 to 20000 to big .h files (like monsym.h) */
/* can be handled. */

#define NCHNAM 63			/* NHFB: was constant 8 in code */
 ...
 * July/August 1978
 *
 * Modified by J.Lepreau 3/81
 * to:	Clean up ugly code appearance a little;
 *	Add ifdefs,etc, for tops20;
 *	Add tops20 filename and directory stuff;
 *	Make tables bigger.
 *
 * NOTE: There is definitely a bug in this program: it does not always
 *	 remember that a symbol was defined!  -fjl
*/
...
# else
/* # define symsiz 1000 */
# define symsiz 20000			/* NHFB: make it big!!! */
# endif
...
#define xmac2(c0,c1,cpos,op)
			...
			for (;;) {
				...
				tmac1(c,b0);
				i =  *p++;
				if (!isid(i)) goto endid;
				tmac1(i,b1);
				tmac2(c,i,0);
				c =  *p++;
				if (!isid(c)) goto endid;
				tmac1(c,b2);
				tmac2(i,c,1);
				i =  *p++;
				if (!isid(i)) goto endid;
...
char *			/* find and handle preprocessor control lines */
...
/****	Next line is fixed per Alan Zaring (alan@utexas-11)		***/
/**	In one of your earliest messages you mentioned that you'd occassionally
/** 	been bothered by cpp losing track of a defined symbol.  We just ran
/**	into a similar problem.
/**	The problem can occur if you #undef a macro when the macro has not
/** 	been #define'd.

/*** 	if (enterf>0) sp->name = namep;		****/
	if (enterf != DROP && enterf > 0) sp->name = namep;
	return(lastsym = sp);
}

struct symtab *
slookup(p1,p2,enterf) register char *p1,*p2;
int enterf;
{
	register char *p3;
	char c2,c3;
	...
	*p2 = '\0';	/* mark end of token */
	if ((p2-p1)>NCHNAM) p3 = p1+NCHNAM;
	else p3 = p2;
	c3 =  *p3;
	*p3 = '\0';	/* truncate to NCHNAM chars or less */
...
char *
trmdir(s) register char *s;		/* remove filename, leaving path */
{
	register char *p = s;
	while (*p++);
	--p;				/* pnt to last real char */
	while (p>s  &&  *--p != '/');
# if unix
	if (p == s) *p++ = '.';		/* if no path given, than path is . */
# endif
	*p = '\0';
	return(s);
}
...
#ifdef tops20
static 	char	badir[] = "Bad directory \"%s\" to -I ignored";

char *index();

int
fixdir(p)			/* cnvt dir to unix format, return 0 if bad */
register char *p;
{
	char	*q;
	char	c;
	...
	if (*p == '<') {		/* verify that is directory */
		if ((q = index(p, '>')) == 0 || *(q+1) != EOS) {
			pperror(badir, p);
			return(0);		/* error */
		}
		*q = EOS;
...
# endif
	...
	char	*p, *q;
		...
		if ((q = index(p = argv[i]+2, ':')) != 0) {
			if (*(q+1) != EOS) {		/* device & directory*/
				if (!fixdir(q+1)) break;	
				*(q+1) = '/';
			}			/* otherwise device only */
			for (;q > p; --q)	/* shift device 1 to right */
				*q = *(q-1);
			*p = '/';
			dirs[nd++] = p;
		}
		else if (*p == '<') {			/* directory only */
			if (!fixdir(p)) break;	/* error, fixdir prts msg */
			dirs[nd++] = p+1;
		}
		else dirs[nd++] = argv[i]+2;		/* unix format */
	}
#else
			...
			default:
				if (fin == STDIN) {	/* cnvt to unix fmt */
					char unixnam[FNMAX];
					fnunix(unixnam, argv[i]);
					if (0 > (fin = open(argv[i], READ))) {
						pperror("No source file %s",argv[i]);
						exit(8);
					}
					fnames[ifno] = copy(unixnam);
					dirs[0] = dirnams[ifno] = trmdir(unixnam);
#ifndef gcos
...
# endif
#if tops20
		dirs[nd++] = "/pcc";	/* pcc: logical name */
		dirs[nd++] = "pcc";	/* <pcc> so can get  <pcc.sys> */
# endif
# if gcos
		dirs[nd++] = "cc/include";
# endif
...
# if ibm
		ysysloc = stsym ("ibm");
# endif
# if tops20 || TOPS20 || twenex		/* synonyms */
		stsym("tops20");
		stsym("TOPS20");	/* for compatibility with Snyder */
		ysysloc = stsym("twenex");
# endif
# if pdp11
		varloc = stsym("pdp11");
# endif
