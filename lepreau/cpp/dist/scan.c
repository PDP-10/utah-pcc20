...
/* no debugging stuff for now, can't control it */
#define YYDEBUG SCAN
#undef	SCAN
...
#if DEBUG & SCAN
char *actnames[] = {
	"A_ERR", "A_LET", "A_DIG", "A_1C", "A_STR", "A_CC",
	"A_BCD", "A_SL", "A_DOT", "A_PL", "A_MI", "A_EQ",
	"A_NOT", "A_LT", "A_GT", "", "A_AND", "A_OR", "A_WS",
	"A_NL"
};
#endif
/*
...
yylex()
{
	for (;;) {
		...
		lxchar = getchar();
		p = lxcp[lxchar+1];
		printf("scan: yylex(), lxchar = %o, action = %s\n", lxchar,
			actnames[p->lxact]);
		switch(p->lxact) {
#else
		...
		onechar:
			...
			case A_LET:
				/*
				 * collect an identifier, check for reserved word,
				 * and return
				 */
			...
			case A_DIG:
					...
					case 'X':
						 ...
						 */
						{
							...
							for (cp = yytext + 2; *cp; ++cp) {
								/*
								 * this code won't work for all
								 * wild character sets,
								 *  but seems ok for ascii and
								 * ebcdic
								 */
								lastcon << = 4;
								if (isdigit (*cp))
				...
				hexlong:
						/*
						 * check for longness in one place
						 */
						goto islong;
			...
			case A_BCD:
				{
					...
					yytext[i] = '\0';
					if (i > 6)
						uerror ("BCD constant exceeds 6 characters");
# ifdef gcos
					else
						strtob (yytext, &lastcon, i);
					lastcon >> = 6 * (6 - i);
# else
...
#if DEBUG & SCAN
	if (yydebug > 1)
		printf("scan: lxres(), starting char = %c\n", p->lxrch[0]);
#endif
	for (p = lxrdope + c; p->lxrch[0] == ch; ++p) {
#if DEBUG & SCAN
		if (yydebug > 1)
			printf("scan: lxres(), strcmp( %s, %s )\n", yytext, p->lxrch);
#endif
		if (!strcmp (yytext, p->lxrch)) {
			...
			switch (p->lxract) {
				...
				case AR_RW: 	/* ordinarily reserved word */
...
lxtitle()
{
	...
	static	int	firstflg = 1;	/* means 1st time called, ie from lxinit() */
	for (;;) {
		/*
